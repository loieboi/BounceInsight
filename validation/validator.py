...
class Validator:
    ...
    def validate(self):
        ...
        self.validate_data(df_fp, df_gym)
    ...
    def validate_data(self, df_fp, df_gym):
        ...
        for col_fp, col_gym in columns_to_compare.items():
            merged_df[f'{col_fp}_diff'] = np.abs(merged_df[f'{col_fp}_fp'] - merged_df[f'{col_gym}_gym'])
            merged_df[f'{col_fp}_within_tolerance'] = merged_df[f'{col_fp}_diff'] <= (
                        self.tolerance * np.maximum(merged_df[f'{col_fp}_fp'], merged_df[f'{col_gym}_gym']))
